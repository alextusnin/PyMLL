    ...
    
{
    ...
    for (int i_det=0; i_det<Ndet; i_det++){
        for (int i_phi=0; i_phi<Nphi; i_phi++){
            res_RE[i_det*Nphi*2+i_phi] = out.ysave[i_phi][i_det];
            res_IM[i_det*Nphi*2+i_phi] = out.ysave[i_phi+Nphi][i_det];
            
            res_RE[i_det*Nphi*2 + Nphi+i_phi] = out.ysave[2*Nphi+i_phi][i_det];
            res_IM[i_det*Nphi*2+Nphi+i_phi] = out.ysave[2*Nphi+i_phi+Nphi][i_det];
